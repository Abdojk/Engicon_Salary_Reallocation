...
# Reporting period — pooled aggregate. Edit these to target a different range.
PERIOD_YEAR = 2026
...
def load_and_filter_ins(path, start_month, end_month, year):
...
def load_and_filter_hours(path, start_month, end_month, year):
    df = pd.read_excel(path, engine="openpyxl")
    df["Project date"] = pd.to_datetime(df["Project date"], errors="coerce")

    keep = (
        (df["Project date"].dt.year == year)
        & (df["Project date"].dt.month >= start_month)
        & (df["Project date"].dt.month <= end_month)
...
def render_project_report(rows, start_month, end_month, year):
...
def render_worker_report(rows, start_month, end_month, year):
...
def build_document(title_en, title_ar, table_html, start_month, end_month, year):
    ar_period = f"{ARABIC_MONTHS[start_month]} {year} – {ARABIC_MONTHS[end_month]} {year}"
    en_period = f"{ENGLISH_MONTHS[start_month]} – {ENGLISH_MONTHS[end_month]} {year}"
    ...
    return (
        ...
        '</div>\n'
        f'<div class="filter-line">Filter: Months={start_month}-{end_month} · Year={year} · CompanyCode=C01 · ProjectCost=Yes</div>\n'
        f'{table_html}\n'
...
def main():
    ...
    year = PERIOD_YEAR
    print(
        f"Reporting period (pooled): "
        f"{ENGLISH_MONTHS[start_month]}–{ENGLISH_MONTHS[end_month]} {year}"
    )
    ...
    ins_df = load_and_filter_ins(ins_path, start_month, end_month, year)
    hours_df = load_and_filter_hours(hours_path, start_month, end_month, year)
    ...
    project_html = render_project_report(project_rows, start_month, end_month, year)
    worker_html = render_worker_report(worker_rows, start_month, end_month, year)

    mmm_start = calendar.month_abbr[start_month]
    mmm_end = calendar.month_abbr[end_month]
    yyyy = str(year)
    out1 = os.path.join(
        BASE_DIR, f"salary_reallocation_by_project_{mmm_start}-{mmm_end}_{yyyy}.html"
    )
    out2 = os.path.join(
        BASE_DIR, f"salary_reallocation_by_worker_{mmm_start}-{mmm_end}_{yyyy}.html"
    )
    ...
    print(f"Period             : {ENGLISH_MONTHS[start_month]}–{ENGLISH_MONTHS[end_month]} {year}")
