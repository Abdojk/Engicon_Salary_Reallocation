...
def prompt_period():
    """Prompt for a From/To month range and return (from_m, from_y, to_m, to_y)."""
    from_month = _read_month("Enter From Month (1-12): ")
    from_year = _read_year("Enter From Year (e.g. 2026): ")
    to_month = _read_month("Enter To Month (1-12): ")
    to_year = _read_year("Enter To Year (e.g. 2026): ")

    from_key = from_year * 100 + from_month
    to_key = to_year * 100 + to_month
    if from_key > to_key:
        print("ERROR: From period cannot be after To period.")
        sys.exit(1)

    span = (to_year - from_year) * 12 + (to_month - from_month) + 1
    if span > 12:
        print("ERROR: Period cannot exceed 12 months.")
        sys.exit(1)
...
def period_label(from_m, from_y, to_m, to_y, lang):
    months = ARABIC_MONTHS if lang == "ar" else ENGLISH_MONTHS
    if (from_m, from_y) == (to_m, to_y):
        return f"{months[from_m]} {from_y}"
    return f"{months[from_m]} {from_y} – {months[to_m]} {to_y}"


def period_slug(from_m, from_y, to_m, to_y):
    start = f"{calendar.month_abbr[from_m]}{from_y}"
    if (from_m, from_y) == (to_m, to_y):
        return start
    return f"{start}-{calendar.month_abbr[to_m]}{to_y}"
...
def load_and_filter_ins(path, from_m, from_y, to_m, to_y):
    ...
    from_key = from_y * 100 + from_m
    to_key = to_y * 100 + to_m
...
def load_and_filter_hours(path, from_m, from_y, to_m, to_y):
    df = pd.read_excel(path, engine="openpyxl")
    df["Project date"] = pd.to_datetime(df["Project date"], errors="coerce")

    from_date = pd.Timestamp(from_y, from_m, 1)
    to_date = pd.Timestamp(to_y, to_m, calendar.monthrange(to_y, to_m)[1])
    proj_date = df["Project date"].dt.normalize()
...
def render_project_report(rows, period):
...
def render_worker_report(rows, period):
...
def build_document(title_en, title_ar, table_html, period):
    from_m, from_y, to_m, to_y = period
    ar_period = period_label(from_m, from_y, to_m, to_y, "ar")
    en_period = period_label(from_m, from_y, to_m, to_y, "en")
    generated = datetime.now().strftime("%d %b %Y %H:%M")
    return (
        '<html dir="auto">\n'
        '<head>\n'
        '<meta charset="UTF-8">\n'
        f'<title>{html.escape(title_ar)} | {html.escape(title_en)} — {en_period}</title>\n'
        f'{STYLE_BLOCK}\n'
        '</head>\n'
        '<body>\n'
        '<div class="title-block">\n'
        '<div class="title-main">تقرير إعادة توزيع الرواتب | Salary Reallocation Report</div>\n'
        f'<div class="title-sub">{ar_period} | {en_period}</div>\n'
        '<div class="title-small">الشركة / Company: Engicon (WNGJ) — CompanyCode: C01</div>\n'
        '</div>\n'
        f'<div class="filter-line">Filter: Period={en_period} · CompanyCode=C01 · ProjectCost=Yes</div>\n'
        f'{table_html}\n'
...
def main():
    ...
    from_m, from_y, to_m, to_y = prompt_period()
    period = (from_m, from_y, to_m, to_y)
    print(f"Reporting period: {period_label(from_m, from_y, to_m, to_y, 'en')}")

    ins_df = load_and_filter_ins(ins_path, from_m, from_y, to_m, to_y)
    hours_df = load_and_filter_hours(hours_path, from_m, from_y, to_m, to_y)
    ...
    project_html = render_project_report(project_rows, period)
    worker_html = render_worker_report(worker_rows, period)

    slug = period_slug(from_m, from_y, to_m, to_y)
    out1 = os.path.join(BASE_DIR, f"salary_reallocation_by_project_{slug}.html")
    out2 = os.path.join(BASE_DIR, f"salary_reallocation_by_worker_{slug}.html")
